...
# faster insertion or shifting in long statements maybe?

# Overall you can pick the version you find easier.
'''
Why not include repr() as an function, that always returns the object as printable.
'''
...
print("\n---- for loops ----")
x = range(0,10)
for i in x: # 10 not included
    print(i)


# what happens if we invoke the list class
print("\n---- for loop with list ----")
x = list(x)
print("type of x: " + repr(type(x)))
for i in x:
    print(i)
...
# 'len' does exactly what you expect, gets the length of something.
...
print("\nprint range of length of this list: \n" + repr(list1))
...
print("\nprint dict: ")
for item in d: #or d.keys(): print(d[item])
    print(repr(item) + ": " +  repr(d[item]))

# we can also do this another way by invoking .items() this returns the key and
# the value.
...
print("\n---- Functions ----")
def rectangle_area(a, b): # how do i define input type?
    return a*b
print("rectangle_area(3,4) = " + repr(rectangle_area(3,4)))

# Along with having multiple inputs you can also use multiple outputs.
...
# now let's create a function that takes as input, radius) and pi
...
# To open a file we need two pieces of information, the file name, and the opening
# properties like read (r), write (w), or append (a).
# fid = open( 'file.txt', 'w' ) # I'll set this to write as I do want to completely
...
with open("file.txt", 'w') as file:
    #automaticly closes because of with, why not teach this on?
    pass
...
# print( <string to write>, file=fid  ) Remember this does put a new line escape automatically
...
with open("file.txt", 'w') as file:
    file.write("Write")
    print("\nPrint" + " float: " + repr(float1) + " integer:" + repr(int1) + "\n", file = file)
...
with open("file.txt", 'r') as file:
    print()
    for line in file.readlines(): # "line in file" works directly
        line = line[:-1]
        print(line)
...
print("\nload and save each line separately")
with open("file.txt", 'wb') as file:
    pickle.dump(list1, file = file)
    pickle.dump(dict1, file = file)
